class InformationController < ApplicationController
  ...
  def index
    @information = if params[:name] || params[:address]
        Information.where('name LIKE ? AND address LIKE ?', "%#{params[:name]}%", "%#{params[:address]}%")
      else
        Information.all
      end
  end
  ...
  private
    ...
    def information_params
      params.require(:information).permit(:name, :infor, :address, :info, :status, :infection, :news, :publication, :district, :province, :yob)
    end
